...
<body class="bg-[#f0f5fb] text-gray-800">
  ...
  <main class="max-w-2xl mx-auto px-4 py-10 sm:py-14">
    ...
    <div class="bg-white rounded-2xl shadow-sm p-6 sm:p-8" id="main-card">
      ...
      <div class="panel" id="panel-2">
        ...
        <div class="mb-6">
          <div class="flex items-center justify-between mb-3">
            <button onclick="prevMonth()" class="w-8 h-8 rounded-full hover:bg-[#dce8f5] flex items-center justify-center text-[#1e3a6e] transition-colors">
            ...
            <button onclick="nextMonth()" class="w-8 h-8 rounded-full hover:bg-[#dce8f5] flex items-center justify-center text-[#1e3a6e] transition-colors">
      ...
      <div class="flex justify-between items-center mt-8 pt-5 border-t border-gray-100">
        <button id="btn-prev" onclick="prevStep()" class="text-sm text-[#8ba4c4] hover:text-[#1e3a6e] font-semibold transition-colors hidden">
          ← Retour
        </button>
        <div class="flex-1"></div>
        <button id="btn-next" onclick="nextStep()" class="btn-primary text-white text-sm font-semibold px-7 py-2.5 rounded-lg">
          Continuer →
        </button>
      </div>
    </div>
    ...
    <div id="success-screen">
      <div class="bg-white rounded-2xl shadow-sm p-8 sm:p-12 text-center">
        ...
        <div class="flex flex-col sm:flex-row gap-3 justify-center">
          ...
          <button onclick="resetForm()" class="border-2 border-[#e2eaf4] text-[#1e3a6e] px-6 py-2.5 rounded-lg text-sm font-semibold hover:border-[#4a90d9] transition-colors">Nouveau rendez-vous</button>
        </div>
      </div>
    </div>
  ...
  <script>
    ...
    // ---- Créneaux ----
    const TAKEN_SLOTS = ['09:00','10:30','14:00','15:30']; // simulated taken
    ...
    function buildCreneaux(containerId, slots) {
      ...
      slots.forEach(slot => {
        ...
        el.textContent = slot;
        const taken = TAKEN_SLOTS.includes(slot) && Math.random() < 0.4;
        if (taken) {
    ...
    function nextStep() {
      ...
      if (currentStep === 4) {
        if (!document.getElementById('rgpd').checked) {
          document.getElementById('err-rgpd').classList.remove('hidden'); return;
        }
        submitForm(); return;
      }
      currentStep++;
